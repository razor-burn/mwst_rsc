 ...
 */
QString LoanManagement::getActiveFilterState() const
{
 ...
 */
void LoanManagement::setActiveFilterState(const QString &value)
{
 ...
 * @param event
 */
void LoanManagement::showEvent(QShowEvent *event)
{
    this->init();
...
/**
 * @brief Konstruktor. Initialisiert alle Member von LoanManagement
 * @param parent
 */
LoanManagement::LoanManagement(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::LoanManagement),
    m_roomsModel(NULL),
...
/**
 * @brief Erstellt die QSqlTableModels (falls noch nicht geschehen) und befüllt die TableViews mit diesen.
 * Überprüft, ob LoanManagement mit einer im Hauptfenster ausgewählten Ausleihe aufgerufen wurde und markiert diese.
 */
void LoanManagement::init()
{
    ...
    if(this->getloansModelRow() != -1)
    {
        if(!dbcon::connecttodb())
            return;
        ui->stackedWidget->setCurrentIndex(1);
        QModelIndexList indexes = m_allLoansModel->match(m_allLoansModel->index(0,0), Qt::DisplayRole, this->getloansModelRow(), 1, Qt::MatchExactly);
        if(indexes.count() != 0)
        {
            this->setloansModelRow(-1);
            this->on_tableViewAllLoans_clicked(indexes[0]);
            ui->tableViewAllLoans->setCurrentIndex(indexes[0]);
            ui->tableViewAllLoans->selectionModel()->select(indexes[0], QItemSelectionModel::Select);

        }
    }
    if(this->getRoomsModelRow() != -1)
    {
        if(!dbcon::connecttodb())
            return;
        ui->stackedWidget->setCurrentIndex(0);
        QModelIndexList indexes = m_roomsModel->match(m_roomsModel->index(0,0), Qt::DisplayRole, this->getRoomsModelRow(), 1, Qt::MatchExactly);
        if(indexes.count() != 0)
        {
            this->setRoomsModelRow(-1);
            this->on_tableViewRooms_clicked(indexes[0]);
            ui->tableViewRooms->setCurrentIndex(indexes[0]);
        }
    }
}
 ...
 */
void LoanManagement::on_radioButtonEditingRooms_toggled(bool checked)
{
    if (checked)
        ui->stackedWidget->setCurrentIndex(0);
}
 ...
 */
void LoanManagement::on_radioButtonResources_toggled(bool checked)
{
    if(checked)
        ui->stackedWidget->setCurrentIndex(1);
}
 ...
 */
void LoanManagement::on_checkBox_toggled(bool checked)
{
    if(!dbcon::connecttodb())
        return;
    if (checked == true)
 ...
 */
void LoanManagement::on_tableViewAllLoans_clicked(const QModelIndex &index)
{
    if(!dbcon::connecttodb())
        return;
 ...
 */
void LoanManagement::on_checkBoxDoneWith_clicked()
{
 ...
 */
void LoanManagement::on_buttonReturnAllPositions_clicked()
{
    QMessageBox::StandardButton result;
 ...
 */
int LoanManagement::setInactive()
{
    if(!dbcon::connecttodb())
        return -1;
...
/**
 * @brief Setter für die auszuwählende Ausleihe bei Ressourcen
 * @param Zeile, die als aktiviert gesetzt werden soll
 */
void LoanManagement::setloansModelRow(int info)
{
    this->m_loansModelRow = info;
}

/**
 * @brief Getter für die auszuwählende Zeile bei Ressourcen
 * @return Die auszuwählende Zeile
 */
int LoanManagement::getloansModelRow() const
{
    return this->m_loansModelRow;
}

/**
 * @brief Setter für die auszuwählende Ausleihe bei Schnitträumen
 * @param Zeile, die als aktiviert gesetzt werden soll
 */
int LoanManagement::getRoomsModelRow() const
{
    return this->m_roomsModelRow;
}

/**
 * @brief Getter für die auszuwählende Zeile bei Schnitträumen
 * @return Die auszuwählende Zeile
 */
void LoanManagement::setRoomsModelRow(int value)
{
    m_roomsModelRow = value;
}

/**
 * @brief Liest die aktuell ausgewählte Ausleihe samt Positionen aus und sendet diese als Druckjob ab.
 */
void LoanManagement::on_buttonPrintPage_clicked()
{
 ...
 */
void LoanManagement::on_buttonPrintPdf_clicked()
{
    QItemSelectionModel * selMod = ui->tableViewAllLoans->selectionModel();
    if(!selMod->hasSelection())
    {
 ...
 * @param index
 */
void LoanManagement::on_tableViewRooms_clicked(const QModelIndex &index)
{
 ...
 */
void LoanManagement::on_buttonSubmitChangesRoom_clicked()
{
 ...
 */
void LoanManagement::on_buttonClearFilter_clicked()
{
    m_roomsModel->setFilter("ende >= NOW()");
    ui->checkBoxSearchByDate->setChecked(false);
}
 ...
 */
void LoanManagement::on_buttonApplyFilter_clicked()
{
    QString filter = "";
 ...
 * @param checked
 */
void LoanManagement::on_checkBoxSearchByDate_toggled(bool checked)
{
    if(checked == true)
    {
 ...
 */
void LoanManagement::on_lineEditSearchResource_textChanged(const QString &arg1)
{
    if(!dbcon::connecttodb())
        return;
 ...
 * @param Suchtext als QString
 */
void LoanManagement::on_lineEditSearchUser_textChanged(const QString &arg1)
{
    if(!dbcon::connecttodb())
        return;
 ...
 */
void LoanManagement::on_pushButtonExtendToDateTime_clicked()
{
    QItemSelectionModel * selMod = ui->tableViewAllLoans->selectionModel();
    if(!selMod->hasSelection())
        return;
